if __name__ == "__main__":
    ...
    ROOT_DIR = os.environ['RL100_CORE_ROOT']
...
class TrainDP3Workspace:
    ...
    def run(self):
        ...
        if not os.path.exists(latest_path) or self.cfg.training.resume == False or (self.cfg.off2off and not self.cfg.off2off_no_bc):
            ...
            for local_epoch_idx in range(max(0, cfg.training.num_epochs - self.epoch)):
                # KL annealing
        ...
        elif cfg.offline or cfg.online:
            dynamics.load(dynamics_path)
        if getattr(cfg, 'stage1_only', False):
        ...
        if cfg.eval:
            ...
            if cfg.offline:
                ...
                print("OFFLINE_POLICY_COMPLETE", flush=True)
        ...
        if cfg.online:
            ...
            print("ONLINE_POLICY_COMPLETE", flush=True)
    ...
    def finetune_dp3(self, dynamics, Q, value, iql, wandb, ema):
        ...
        for step in tqdm.tqdm(range(int(self.cfg.unio4.bppo_steps)), desc='bppo updating ......'):
            ...
            # PPO likelihoods must use the same deterministic neural mapping.
            ...
            if (step+1)% self.cfg.unio4.eval_step == 0:
                ...
                if self.cfg.unio4.is_update_old_policy:
                    if current_mean_qs[0] > best_mean_qs[0]:
    ...
    def online_ft(self, dynamics, Q, value, iql, iql_online, copy_encoder, wandb, ema):
        from dual_rl_buffer import ReplayBuffer
    ...
    def _online_ft_vec(self, dynamics, Q, iql, iql_online, wandb, online_ft_path, cm_optimizer, cm_lr_scheduler, ema=None, reward_scaler_template=None):
        ...
        from dual_rl_buffer import ReplayBuffer as VecReplayBuffer
        from dual_rl_buffer import ReplayBuffer as FlatReplayBuffer
        ...
        if use_subproc_vec_rollout:
            ...
            for env_idx, env in enumerate(envs):
                env.seed(1000000 + int(self.cfg.training.seed) * 100000 + env_idx * 10000)
        ...
        replay_buffer.reset()
        executed_steps_buffer = np.zeros((steps_per_update, train_env_num, 1), dtype=np.float32)
        ...
        # Small paired startup check; periodic evaluations retain the original episode count.
        full_eval_episodes = env_runner.eval_episodes
        env_runner.eval_episodes = full_eval_episodes
        self._repair_initial_phase = True
        # --- initial eval ---
        all_success_rates, all_returns = [], []
        cm_all_success_rates, cm_all_returns = [], []
        all_idql_success_rates, all_idql_returns = [], []
        ...
        self._repair_initial_phase = False
        env_runner.eval_episodes = full_eval_episodes
        # --- main loop state ---
        total_steps = 0
        evaluate_num = 0
        ...
        while total_steps < self.cfg.ppo.max_train_steps:
            ...
            chunk_lengths = np.zeros(train_env_num, dtype=np.int32)
            ...
            if use_subproc_vec_rollout:
                ...
                for i in range(train_env_num):
                    ...
                    chunk_lengths[i] = int(info["executed_steps"])
                    episode_rewards[i] += reward
                    episode_steps_per_env[i] += 1

                    dw = (true_terminal(done, info) if getattr(self.cfg.ppo, 'repair_timeout', False) else done and episode_steps_per_env[i] != max_steps)
            ...
            if replay_buffer.count == steps_per_update:
                ...
                with torch.no_grad():
                    ...
                    adv, v_target = compute_gae_per_env(
                        ...
                        executed_steps=torch.as_tensor(executed_steps_buffer, device=self.device))
                ...
                flat_replay.load_flattened(replay_buffer)
    ...
    def eval(self, online=False, eval_times=1, use_cm=False, distill2mean=False, policy_override=None, eval_name='Eval'):
        ...
        # Reuse the just-completed six-episode evaluation only before any update.
        if getattr(self, '_repair_initial_phase', False) and getattr(cfg.ppo, 'repair_initial_eval_cache', False):
            import json as repair_json
            policy.eval()
            with open(os.path.join(self.output_dir, 'repair_initial_eval.json')) as f:
                cached = repair_json.load(f)[eval_name]
            print(f'REPAIR reused six-episode startup {eval_name}: {cached}', flush=True)
            return cached
        # VIB: temporarily disable stochastic sampling for deterministic eval.
        saved_force_stochastic = []
        ...
        log_data['test_mean_score'] = np.mean(log_data['test_mean_score'])
        log_data['mean_returns'] = np.mean(log_data['mean_returns'])
        if online and policy_override is None and not use_cm:
            best_dir = os.path.join(self.output_dir, 'online_best')
            self._maybe_update_best(
                score=float(log_data['test_mean_score']), best_dir=best_dir,
                best_score_path=os.path.join(best_dir, 'best_score.csv'),
                lock_path=os.path.join(self.output_dir, '.online_best.lock'),
                save_fn=self.unio4.save, eval_name='Online policy')
...
@hydra.main(
    version_base=None,
    config_path=str(pathlib.Path(os.environ['RL100_CONFIG_ROOT']))
)
def main(cfg):
