...
from dual_env import DualViewPegEnv as MetaWorldEnv
...
class MetaworldRunner(BaseRunner):
    ...
    def _stack_obs(self, obs_list, device):
        ...
        batched = {}
        for key in ['image_corner2', 'image_behindGripper', 'agent_pos']:
    ...
    def run(self, policy: BasePolicy, use_cm=False, distill2mean=False, eval_env_num=1):
        ...
        self.env.seed(getattr(self, 'eval_seed_base', 700000))
        ...
        while episode_idx < self.eval_episodes:
            ...
            for j in range(wave_size):
                envs[j].seed(getattr(self, 'eval_seed_base', 700000) + episode_idx + j)
    ...
    def _run_serial(self, policy: BasePolicy, use_cm=False, distill2mean=False):
        ...
        for episode_idx in tqdm.tqdm(range(self.eval_episodes), desc=f"Eval in Metaworld {self.task_name} Pointcloud Env", leave=False, mininterval=self.tqdm_interval_sec):
            ...
            while not done:
                ...
                with torch.no_grad():
                    obs_dict_input = {
                        'image_corner2': obs_dict['image_corner2'].unsqueeze(0).to(torch.float),
                        'image_behindGripper': obs_dict['image_behindGripper'].unsqueeze(0).to(torch.float),
                        'agent_pos': obs_dict['agent_pos'].unsqueeze(0).to(torch.float),
                    }
