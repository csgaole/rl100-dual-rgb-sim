...
class MetaworldMultiViewDataset(BaseDataset):
    def __init__(self,
        ...
        # Use the collection-time split stored in the dataset so every stage
        # and seed sees the identical 90/10 partition.
        root = zarr.open_group(zarr_path, mode='r')
        stored_train = root['meta/train_episode_indices'][:]
        self.train_mask = np.zeros(self.replay_buffer.n_episodes, dtype=bool)
        self.train_mask[stored_train] = True
        self.sampler = SequenceSampler(
            replay_buffer=self.replay_buffer,
            sequence_length=horizon,
            pad_before=pad_before,
    ...
    def get_normalizer(self, mode='limits', **kwargs):
        ...
        indices = np.concatenate([
            np.arange(starts[i], ends[i]) for i in np.flatnonzero(self.train_mask)
        ])
        data = {
            'action': self.replay_buffer['action'][indices],
            'agent_pos': self.replay_buffer['state'][indices],
            'next_action': self.replay_buffer['next_action'][indices],
            'next_agent_pos': self.replay_buffer['next_state'][indices],
        }
