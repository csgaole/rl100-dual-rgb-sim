...
def compute_gae_per_env(rewards, dones, dws, vs, vs_, gamma, lamda, n_action_steps=1, executed_steps=None):
    """
    ...
    gamma_eff = gamma ** n_action_steps if executed_steps is None else gamma ** executed_steps
    ...
    for t in reversed(range(steps)):
        gae = deltas[t] + (gamma_eff if executed_steps is None else gamma_eff[t]) * lamda * gae * (1.0 - dones[t])
        adv[t] = gae
...
class BehaviorProximalPolicyOptimization(ProximalPolicyOptimization):
    ...
    def NStepValueEstimation(self, nobs_features, nactions, dynamics, value, Q, iql, opt_steps):
        ...
        history = nobs_features.reshape(batch_size, self.cfg.n_obs_steps, -1)
        for i in range(opt_steps):
            advantage = self.advantage_computation(history.reshape(batch_size, -1), nactions[:, i], value, Q, iql)
            advantages.append(advantage)
            next_features, reward, _, _ = dynamics.step(history[:, -1], nactions[:, i], history)
            rewards.append(torch.as_tensor(reward, device=self._device))
            next_features = torch.as_tensor(next_features, device=self._device, dtype=history.dtype)
            history = next_features if dynamics.prediction_mode == 'full' else torch.cat((history[:, 1:], next_features.unsqueeze(1)), dim=1)
            ...
            with torch.no_grad():
                ...
                for batch_size in [4, 16]:
                    ...
                    for start in range(0, 16, batch_size):
                        ...
                        features = self._policy.obs2latent(dict_apply(states, lambda x: x[start:stop]), **({'training': False} if getattr(self.cfg, "feature_type", "3D") == '2D' else {}))
                        for i, t in enumerate(self._policy.noise_scheduler.timesteps):
                            ts = torch.as_tensor(t, device=self._device).reshape(1).expand(batch_size)
                            output = self._policy.model(sample=actions[i,start:stop], timestep=self._policy.get_unet_timesteps(ts), local_cond=None, global_cond=features)
                            now, _ = self._policy.noise_scheduler.step_forward_logprob_with_entropy(output, ts, actions[i,start:stop], next_sample=actions[i+1,start:stop], eta=self.args.eta)
                            error = (now.sum(-1) - old_lp[i,start:stop].sum(-1)).abs().max().item()
                            assert np.isfinite(error)
                            worst = max(worst, error)
                    errors[batch_size] = worst
                assert errors[4] < 1e-3, f'Same-batch likelihood invariance failed: {errors}'
                assert errors[16] < 1e-2, f'Cross-batch likelihood invariance failed: {errors}'
                print(f'REPAIR live likelihood invariance PASS max_abs_logratio_by_batch={errors}', flush=True)
                ...
                for i, t in enumerate(self._policy.noise_scheduler.timesteps):
                    ...
                    if not self.args.fix_encoder and self.args.recon and (self.args.per_step_recon or i == 0):
                        ...
                        with torch.set_grad_enabled(not self.args.fix_encoder):
                            obs_feature = self._policy.obs2latent(state, **({"training": not getattr(self.args, "repair_probability", False)} if getattr(self.cfg, "feature_type", "3D") == "2D" else {}))
                    ...
                    if self.cfg.chunk_as_single_action or getattr(self.args, "joint_chunk_ratio", True):
